...
const print_vulkan_objects = struct {
    ...
    const memory_type_all: bool = false;
    /// Capabilities of the selected surface
    /// VSync, transparency, etc
    const surface_abilties: bool = false;
};
...
fn draw() !void {
    ...
    const extent = geometry.Extent2D(f32){
        ...
        .height = (@intToFloat(f32, 512) / @intToFloat(f32, screen_dimensions.height)) * 2.0 * enlarge_by,
    };
    ...
    // };
    // _ = try generateText(graphics.GenericVertex, &face_writer, "E", .{ .x  = -1.0, .y = -1.0 }, scale_factor, glyphs, icon_color.toRGBA(), null);
    ...
    //         _ = try generateText(graphics.GenericVertex, &face_writer, "E", .{ .x  = extent.x, .y = extent.y }, scale_factor, glyphs, icon_color.toRGBA(), null);
    //     }
    // }
    vertex_buffer_quad_count = 2; //  + (horizonal_count * vertical_count);
}
...
fn setup(allocator: std.mem.Allocator, app: *GraphicsContext) !void {
    ...
    if (is_staging_buffer_required) {
        ...
        // const len = @intCast(u64, texture_layer_dimensions.width) * texture_layer_dimensions.height;
        // std.debug.assert(len == glyphs.image.len);
    ...
    for (text_buffer) |char, i| {
        ...
        if (char != ' ') {
            const glyph_index = blk: {
                ...
                std.log.err("Character not in set '{c}' (ascii {d}) in '{s}'", .{ char, char, text_buffer });
                break :blk default_char_index;
                // return error.InvalidCharacter;
            };

            const texture_extent = try glyph_set.imageRegionForGlyph(glyph_index, texture_layer_dimensions);
            ...
            const char_extent = geometry.Extent2D(ScreenNormalizedBaseType){
            ...
            vertices[i - skipped_count] = graphics.generateTexturedQuad(VertexType, char_extent, texture_extent, .bottom_left);
...
pub const GlyphSet = struct {
    ...
    pub fn deinit(self: GlyphSet, allocator: std.mem.Allocator) void {
...
fn createGlyphSet(
    ...
    const scale = geometry.Scale2D(f32){
        // TODO
        .x = font.scaleForPixelHeight(font_info, 96),
        .y = font.scaleForPixelHeight(font_info, 96),
    };
    ...
    for (character_list) |char, i| {
        ...
        const bounding_box = try font.getCodepointBoundingBoxScaled(font_info, char, scale);
    ...
    glyph_set.image = try allocator.alloc(graphics.RGBA(f32), @intCast(u64, texture_dimensions.height) * texture_dimensions.width);
        ...
        while (y < max_height) : (y += 1) {
            ...
            while (x < max_width) : (x += 1) {
                ...
                if (!background_pixel) {
                    ...
                    glyph_set.image[pixel_index] = graphics.RGBA(f32).clear();
                }
            }
        }
    }
